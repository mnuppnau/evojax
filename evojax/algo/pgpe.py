...
@jax.jit
def get_activation_mean(
    activations: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]
...
@jax.jit
def scale_activation_grads(
    activation_grads: jnp.ndarray, target_norm: float
) -> jnp.ndarray:
...
def ask_func(
    ...
    solutions = jnp.hstack([center + scaled_noises, center - scaled_noises]).reshape(
        -1, solution_size
    )
...
class PGPE(NEAlgorithm):
    ...
    def __init__(
        ...
        self._lr_decay_steps = optimizer_config.get("center_lr_decay_steps", 1000)
        ...
        self.belief_space = initialize_belief_space(
    ...
    def tell(
        self,
        fitness: Union[np.ndarray, jnp.ndarray],
        activations: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray],
    ) -> None:
        ...
        activation_grads = jax.grad(activation_loss_fn)(activations, prev_activations)

        processed_activation_grads = process_activation_grads_to_fit_params(
            activation_grads, self._center.shape, self._center.shape[0]
        )

        self.fitness_scores, self._best_score = process_scores(
            fitness, self._solution_ranking
        )
        ...
        processed_activation_grads = scale_activation_grads(
            processed_activation_grads, jnp.linalg.norm(grad_center)
        )
        ...
        if self._t > 100 and self._t < 200:
            self.belief_space = add_ind_topographic_ks(
                self.belief_space, grad_center_norm, grad_stdev_norm, best_score
            )
        elif self._t >= 200:
        ...
        with open("/home/gh0st/Downloads/pgpe_ks_weights.csv", "a") as f:
            f.write(
                f"Iter: {self._t}, Domain Weight: {ks_weights[0]}, Situational Weight: {ks_weights[1]}, History Weight: {ks_weights[2]}, Topographic Weight: {ks_weights[3]}\n"
            )
        ...
        self.belief_space = update_knowledge_sources(
            self.belief_space,
            (
                (self._center * 0.8 + best_individual[0] * 0.2),
                self._stdev,
                best_individual[2],
            ),
            activations,
