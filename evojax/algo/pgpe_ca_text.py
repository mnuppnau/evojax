...
"""IMDb hard-attention fork of `pgpe_ca.py`.

Same PGPE+CA mechanics as the BloodMNIST solver, but with the morphology-
specific fitness components and CA semantic-trap signals replaced by the
attention-quality metrics from `evojax.task.attention_metrics`.

Differences vs. `pgpe_ca.py`:
  - Class is `PGPE_CA_Text` (not `PGPE`).
  - `tell()` takes attention metrics
    (`sparsity_band_reward`, `span_continuity`, `code_disjointness`,
    `position_entropy`, `pos_neg_attention_overlap`)
    in place of the BloodMNIST morphology bag (`cell_circularity`,
    `nucleus_offset`, etc.).
  - Fitness composition uses `code_disjointness` (already in higher-is-
    better direction; do NOT call the standalone `per_code_attention_overlap`
    here — see `attention_metrics.compute_all` docstring).
  - `bio_score` for the Historical KS archive is replaced by an
    attention-quality score so KS rescue still has a task-specific signal
    when the framework is re-enabled in a future phase.
  - `update_normative_ks` is fed attention signals
    (span_continuity, position_entropy, sparsity_density) in the slots that
    used to take cell metrics. Their semantic role (3 task-specific signals
    that drive normative bound learning) carries over even though the
    interpretation differs.
  - For the IMDB-B00 baseline: instantiate with
    `ca_blend_coeff=0.0, static_fitness_weights=True` so the entire CA
    intervention surface is dormant; the metrics are still logged for Phase 9
    analysis but do not steer the gradient.
  - Future logit-prior CA integration will hook into `tell()` after the
    fitness composition (see TODO marker).
...
class PGPE_CA_Text(NEAlgorithm):
    """PGPE+CA solver for the IMDb hard-attention HyperNet-InfoGAN.

    Same algorithmic skeleton as `evojax.algo.pgpe_ca.PGPE` but the fitness
    composition and CA-facing metrics are attention-quality signals instead
    of BloodMNIST morphology metrics. See module docstring for the full
    rundown of differences.
    """
    ...
    def __init__(
        ...
        if logger is None:
            self._logger = create_logger(name="PGPE_CA_Text")
        else:
            self._logger = logger
    ...
    def tell(
        ...
        # Attention metrics from `evojax.task.attention_metrics.compute_all`.
        # `code_disjointness` is already in higher-is-better direction; do
        # not pass `per_code_attention_overlap` here for fitness purposes.
        #
        # NOTE for Phase 7 wiring: `disc_logits` is consumed by
        # `jnp.mean(disc_logits, axis=(0, 1))` later in this function to
        # reduce to a per-code (n_codes,) vector for the population-entropy
        # proxy. That requires a 3D shape `(pop_size, batch, n_codes)`. For
        # IMDb this is the Q-head logits stacked across the population
        # axis by vmap; pass them through directly without batch-averaging.
        sparsity_band_reward: Union[np.ndarray, jnp.ndarray],
        span_continuity: Union[np.ndarray, jnp.ndarray],
        code_disjointness: Union[np.ndarray, jnp.ndarray],
        position_entropy: Union[np.ndarray, jnp.ndarray],
        sparsity_density: Union[np.ndarray, jnp.ndarray],
        pos_neg_attention_overlap: Union[np.ndarray, jnp.ndarray],
       ...
       ## 3. Calculate Fitness
        ...
        # Attention-quality fitness terms (replace BloodMNIST morphology block).
        # Live for IMDb baseline because they shape the discrete mask geometry.
        # `code_disjointness` is already higher-is-better (it's `1 - overlap`)
        # so it goes through `rank_normalize` like the other reward terms.
        #
        # Re-tuned 2026-05-17 after the 200-iter Phase 8 smoke showed PGPE
        # finding a degenerate solution: each code picked ~1 different token,
        # which maximized disjointness while crashing sparsity to <0.02 and
        # leaving f_adv/f_mi at chance. The new balance gives the band
        # penalty more authority and reduces the disjointness pull.
        w_sparsity_band = jnp.float32(0.25)   # ↑ from 0.10 — must compete with disjoint
        w_span_continuity = jnp.float32(0.10) # ↑ from 0.06 — favor coherent spans
        w_disjoint = jnp.float32(0.04)        # ↓ from 0.10 — discourage trivial 1-token-per-code

        # 3. Calculate Fitness (all rank_normalize for scale parity)
        fitness_scores = (
            (rank_normalize(fitness_adv) * w_adv)              # Classifier accuracy
            + (rank_normalize(fitness_mi) * w_mi)              # MI(z, mask)
            + (rank_normalize(r_sense) * w_sense)              # Feature-space code separation
            + (rank_normalize(pop_var) * w_div)                # Population diversity
            + (rank_normalize(r_intra) * w_intra)              # Within-code consistency
            + (rank_normalize(sparsity_band_reward) * w_sparsity_band)  # mask density in [low, high]
            + (rank_normalize(span_continuity) * w_span_continuity)     # contiguous attention spans
            + (rank_normalize(code_disjointness) * w_disjoint)          # codes pick disjoint subsets
        )

        # TODO (future logit-prior CA): once we move from gradient-blend to
        # logit-prior intervention, the CA will produce per-token additive
        # logits at sample time (not at PGPE-update time). The hook lives in
        # the policy / sampler, not here in the solver. This solver class
        # remains responsible only for fitness composition + the legacy
        # gradient-blend path (kept for ablation comparability, OFF for
        # baseline via `ca_blend_coeff=0.0`).
        ...
        # Normative KS receives 3 task-specific signals in the slots that
        # used to take cell metrics. The role (drive normative bound learning
        # from elite-quantile signals) carries over; the interpretation
        # changes from "cell shape band" to "attention shape band":
        #   span_continuity   — favors longer contiguous attention runs
        #   position_entropy  — penalizes positional shortcuts
        #   sparsity_density  — keeps mask density inside a learned band
        # CA is OFF in baseline (ca_blend_coeff=0.0), so these bounds are
        # learned and logged but do not steer the gradient.
        #
        # TODO (Phase 9 cleanup): the clip bounds inside
        # `update_normative_ks` (knowledge_sources.py ~lines 769-773) are
        # hardcoded for BloodMNIST morphology ranges:
        #   cell_circularity_floor   ∈ [0.45, 0.80]
        #   cell_area_var_ceiling    ∈ [0.001, 0.01]
        #   nucleus_offset           ∈ [0.05, 0.55]
        # When the slots are reused for attention metrics, the clips will
        # produce nonsensical normative bounds (span_continuity values can
        # exceed 10, position_entropy lives in [0, 1], sparsity_density in
        # [0, 1]). Acceptable for IMDB-B00 because CA is off; the
        # `norm_*_floor`/`_ceiling` diagnostics are NOT meaningful in
        # baseline runs and should be ignored. Re-enable the CA gradient
        # path only after refactoring `update_normative_ks` to accept
        # task-specific clip ranges (or fork the helper too).
        ...
        # PGPE layout: solutions[0..N-1] = center + noise, solutions[N..2N-1] = center - noise
        ...
        # Attention-quality score used as the Historical KS rescue signal
        # (replaces BloodMNIST's `bio_score`). Same role: a task-specific
        # quality metric the Historical KS prefers when picking which past
        # elites to bring back during a rescue.
        # Scale guard: `sparsity_band_reward` and `code_disjointness` live
        # in [0, 1], but `span_continuity` is an unbounded run length and
        # routinely lands in [1, 10+] at IMDb seq_len=256. Normalize it by
        # a "good span length" anchor (8.0) and clip to [0, 1] so the three
        # components contribute on comparable scales. The absolute scale
        # matters less than relative ordering for KS rescue, but keeping
        # the components balanced makes Phase 9 attribution easier.
        span_norm_anchor = jnp.float32(8.0)
        best_bio_score = jnp.array([
            (1.0 / 3.0) * sparsity_band_reward.flatten()[best_idx]
            + (1.0 / 3.0) * jnp.clip(
                span_continuity.flatten()[best_idx] / span_norm_anchor,
                0.0, 1.0,
            )
            + (1.0 / 3.0) * code_disjointness.flatten()[best_idx]
        ])
        ...
        # The morphology-named slots in `update_metric_history` (which are
        # kwargs with `0.0` defaults — see knowledge_sources.py) are reused
        # to carry attention metrics so the existing buffer + slope code can
        # be shared with the BloodMNIST solver. The naming is misleading
        # inside `metric_history` but no production code reads those slots
        # by name in the IMDb path.
        self.belief_space = update_metric_history(
            ...
            # Reuse slots: dark_range -> sparsity_density,
            #              code_proto_corr -> 1 - code_disjointness (so semantic-trap
            #              "high prototype correlation" check still triggers when
            #              codes converge in mask space),
            #              proto_angle_spread -> position_entropy.
            avg_morph_dark_range=jnp.mean(sparsity_density),
            avg_code_proto_corr=jnp.mean(1.0 - code_disjointness),
            avg_proto_angle_spread=jnp.mean(position_entropy),
            # Bio metrics are placeholders here; they shape the KS scores
            # only when the morphology-aware A04 trap detector fires, which
            # we do not enable in IMDb baseline.
            avg_nuc_cell_ratio_range=jnp.mean(sparsity_band_reward),
            avg_nuc_eccentricity_range=jnp.mean(span_continuity),
            avg_cell_circularity=jnp.mean(span_continuity),
            avg_cell_area_var=jnp.mean(pos_neg_attention_overlap),
            avg_nucleus_offset=jnp.mean(sparsity_density),
        )
        ...
        self._debug_last = {
            ...
            # Slope buffers reused for attention metrics (see update_metric_history
            # call earlier — `dark_range` slot now carries sparsity_density,
            # `code_corr` slot carries 1 - code_disjointness, `angle_spread`
            # slot carries position_entropy). Renamed here for log clarity.
            "sparsity_density_short": dark_range_short,
            "code_overlap_short": code_corr_short,
            "position_entropy_short": angle_spread_short,
            "sparsity_density_latest": dark_range_latest,
            "code_overlap_latest": code_corr_latest,
            "position_entropy_latest": angle_spread_latest,
            "span_continuity_latest": cell_circularity_latest,
            "pos_neg_overlap_latest": cell_area_var_latest,
            "sparsity_density_norm_latest": nucleus_offset_latest,
            "sparsity_band_latest": nuc_ratio_latest,
            "span_continuity_alt_latest": nuc_ecc_latest,
            # NOTE: this value is computed inside `compute_semantic_trap_state`
            # using the original BloodMNIST formula (weighted sum over the
            # metric-history buffer slots that we reuse for attention metrics).
            # Since `span_continuity` (an unbounded run length) sits in two
            # slots with combined weight 0.50, this value will routinely
            # exceed 1.0 and is NOT directly interpretable as an attention
            # quality score. The genuinely-normalized attention quality value
            # is `best_bio_score` inside tell() (sent to the Historical KS
            # archive but not exposed as a diagnostic).
            "bio_proxy_latest_raw": bio_score_latest,
            # Normative bounds learned over attention signals (slots reused).
            "norm_span_continuity_floor": norm_cell_circularity_floor,
            "norm_position_entropy_ceiling": norm_cell_area_var_ceiling,
            "norm_sparsity_density_low": norm_nucleus_offset_low,
            "norm_sparsity_density_high": norm_nucleus_offset_high,
            "norm_span_violation": norm_circ_violation,
            "norm_position_entropy_violation": norm_area_violation,
            "norm_sparsity_violation": norm_offset_violation,
            "norm_attention_violation": norm_morph_violation,
            ...
            # Attention-quality diagnostics (replaces BloodMNIST morphology block).
            "sparsity_band_reward_avg": jnp.mean(sparsity_band_reward),
            "span_continuity_avg": jnp.mean(span_continuity),
            "code_disjointness_avg": jnp.mean(code_disjointness),
            "position_entropy_avg": jnp.mean(position_entropy),
            "sparsity_density_avg": jnp.mean(sparsity_density),
            "pos_neg_attention_overlap_avg": jnp.mean(pos_neg_attention_overlap),
            "w_sparsity_band": w_sparsity_band,
            "w_span_continuity": w_span_continuity,
            "w_disjoint": w_disjoint,
